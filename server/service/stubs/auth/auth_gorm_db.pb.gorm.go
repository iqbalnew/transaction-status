...
type AuthenticationORM struct {
	...
	GeneratePassword string `gorm:"column:generate_password"`
	InitCred         string `gorm:"column:init_cred;type:text"`
	Password         string `gorm:"column:password"`
	Salt             string `gorm:"column:salt;type:text"`
	UserID           uint64 `gorm:"column:user_id;primary_key;not null"`
	Username         string `gorm:"column:username"`
	Verification1    string `gorm:"column:verification_1;type:text"`
	Verification2    string `gorm:"column:verification_2;type:text"`
}
...
func (m *Authentication) ToORM(ctx context.Context) (AuthenticationORM, error) {
	...
	to.Verification1 = m.Verification1
	to.Verification2 = m.Verification2
	to.GeneratePassword = m.GeneratePassword
	to.Salt = m.Salt
	to.InitCred = m.InitCred
...
type UserLoginTimeORM struct {
	Attempts      uint64     `gorm:"column:attempts;type:integer;default:0"`
	LastLoginTime *time.Time `gorm:"column:last_login_time;default:CURRENT_TIMESTAMP"`
	UserID        uint64     `gorm:"column:user_id;primary_key;not null"`
	Username      string     `gorm:"column:username"`
}
...
// TableName overrides the default tablename generated by GORM
func (PasswordResetORM) TableName() string {
	return "password_reset"
}

// ToORM runs the BeforeToORM hook if present, converts the fields of this
// object to ORM format, runs the AfterToORM hook, then returns the ORM object
func (m *PasswordReset) ToORM(ctx context.Context) (PasswordResetORM, error) {
	to := PasswordResetORM{}
	var err error
	if prehook, ok := interface{}(m).(PasswordResetWithBeforeToORM); ok {
		if err = prehook.BeforeToORM(ctx, &to); err != nil {
			return to, err
		}
	}
	...
	if posthook, ok := interface{}(m).(PasswordResetWithAfterToORM); ok {
		err = posthook.AfterToORM(ctx, &to)
	}
	return to, err
}
...
// ToPB runs the BeforeToPB hook if present, converts the fields of this
// object to PB format, runs the AfterToPB hook, then returns the PB object
func (m *PasswordResetORM) ToPB(ctx context.Context) (PasswordReset, error) {
	to := PasswordReset{}
	var err error
	if prehook, ok := interface{}(m).(PasswordResetWithBeforeToPB); ok {
		if err = prehook.BeforeToPB(ctx, &to); err != nil {
			return to, err
		}
	}
	...
	if posthook, ok := interface{}(m).(PasswordResetWithAfterToPB); ok {
		err = posthook.AfterToPB(ctx, &to)
	}
	return to, err
}
...
// The following are interfaces you can implement for special behavior during ORM/PB conversions
// of type PasswordReset the arg will be the target, the caller the one being converted from

// PasswordResetBeforeToORM called before default ToORM code
type PasswordResetWithBeforeToORM interface {
	BeforeToORM(context.Context, *PasswordResetORM) error
}

// PasswordResetAfterToORM called after default ToORM code
type PasswordResetWithAfterToORM interface {
	AfterToORM(context.Context, *PasswordResetORM) error
}

// PasswordResetBeforeToPB called before default ToPB code
type PasswordResetWithBeforeToPB interface {
	BeforeToPB(context.Context, *PasswordReset) error
}

// PasswordResetAfterToPB called after default ToPB code
type PasswordResetWithAfterToPB interface {
	AfterToPB(context.Context, *PasswordReset) error
}
...
func DefaultApplyFieldMaskAuthentication(ctx context.Context, patchee *Authentication, patcher *Authentication, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*Authentication, error) {
	...
	for _, f := range updateMask.Paths {
		...
		if f == prefix+"Verification1" {
			patchee.Verification1 = patcher.Verification1
			continue
		}
		if f == prefix+"Verification2" {
			patchee.Verification2 = patcher.Verification2
			continue
		}
		if f == prefix+"GeneratePassword" {
			patchee.GeneratePassword = patcher.GeneratePassword
			continue
		}
		if f == prefix+"Salt" {
			patchee.Salt = patcher.Salt
			continue
		}
		if f == prefix+"InitCred" {
			patchee.InitCred = patcher.InitCred
			continue
		}
	}
	if err != nil {
...
// DefaultCreatePasswordReset executes a basic gorm create call
func DefaultCreatePasswordReset(ctx context.Context, in *PasswordReset, db *gorm.DB) (*PasswordReset, error) {
	if in == nil {
		return nil, errors.NilArgumentError
	}
	ormObj, err := in.ToORM(ctx)
	...
	if hook, ok := interface{}(&ormObj).(PasswordResetORMWithBeforeCreate_); ok {
		if db, err = hook.BeforeCreate_(ctx, db); err != nil {
			return nil, err
		}
	}
	...
	if hook, ok := interface{}(&ormObj).(PasswordResetORMWithAfterCreate_); ok {
		if err = hook.AfterCreate_(ctx, db); err != nil {
			return nil, err
		}
	}
...
type PasswordResetORMWithBeforeCreate_ interface {
	BeforeCreate_(context.Context, *gorm.DB) (*gorm.DB, error)
}
type PasswordResetORMWithAfterCreate_ interface {
	AfterCreate_(context.Context, *gorm.DB) error
}

func DefaultReadPasswordReset(ctx context.Context, in *PasswordReset, db *gorm.DB) (*PasswordReset, error) {
	if in == nil {
		return nil, errors.NilArgumentError
	}
	ormObj, err := in.ToORM(ctx)
	...
	if hook, ok := interface{}(&ormObj).(PasswordResetORMWithBeforeReadApplyQuery); ok {
		if db, err = hook.BeforeReadApplyQuery(ctx, db); err != nil {
			return nil, err
		}
	}
	if db, err = gorm1.ApplyFieldSelection(ctx, db, nil, &PasswordResetORM{}); err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(PasswordResetORMWithBeforeReadFind); ok {
		if db, err = hook.BeforeReadFind(ctx, db); err != nil {
			return nil, err
		}
	}
	ormResponse := PasswordResetORM{}
	if err = db.Where(&ormObj).First(&ormResponse).Error; err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormResponse).(PasswordResetORMWithAfterReadFind); ok {
		if err = hook.AfterReadFind(ctx, db); err != nil {
			return nil, err
		}
	}
...
type PasswordResetORMWithBeforeReadApplyQuery interface {
	BeforeReadApplyQuery(context.Context, *gorm.DB) (*gorm.DB, error)
}
type PasswordResetORMWithBeforeReadFind interface {
	BeforeReadFind(context.Context, *gorm.DB) (*gorm.DB, error)
}
type PasswordResetORMWithAfterReadFind interface {
	AfterReadFind(context.Context, *gorm.DB) error
}

func DefaultDeletePasswordReset(ctx context.Context, in *PasswordReset, db *gorm.DB) error {
	if in == nil {
		return errors.NilArgumentError
	}
	ormObj, err := in.ToORM(ctx)
	...
	if hook, ok := interface{}(&ormObj).(PasswordResetORMWithBeforeDelete_); ok {
		if db, err = hook.BeforeDelete_(ctx, db); err != nil {
			return err
		}
	}
	err = db.Where(&ormObj).Delete(&PasswordResetORM{}).Error
	if err != nil {
		return err
	}
	if hook, ok := interface{}(&ormObj).(PasswordResetORMWithAfterDelete_); ok {
		err = hook.AfterDelete_(ctx, db)
	}
	return err
}

type PasswordResetORMWithBeforeDelete_ interface {
	BeforeDelete_(context.Context, *gorm.DB) (*gorm.DB, error)
}
type PasswordResetORMWithAfterDelete_ interface {
	AfterDelete_(context.Context, *gorm.DB) error
}

func DefaultDeletePasswordResetSet(ctx context.Context, in []*PasswordReset, db *gorm.DB) error {
	if in == nil {
		return errors.NilArgumentError
	}
	var err error
	...
	if hook, ok := (interface{}(&PasswordResetORM{})).(PasswordResetORMWithBeforeDeleteSet); ok {
		if db, err = hook.BeforeDeleteSet(ctx, in, db); err != nil {
			return err
		}
	}
	err = db.Where("token in (?)", keys).Delete(&PasswordResetORM{}).Error
	if err != nil {
		return err
	}
	if hook, ok := (interface{}(&PasswordResetORM{})).(PasswordResetORMWithAfterDeleteSet); ok {
		err = hook.AfterDeleteSet(ctx, in, db)
	}
	return err
}

type PasswordResetORMWithBeforeDeleteSet interface {
	BeforeDeleteSet(context.Context, []*PasswordReset, *gorm.DB) (*gorm.DB, error)
}
type PasswordResetORMWithAfterDeleteSet interface {
	AfterDeleteSet(context.Context, []*PasswordReset, *gorm.DB) error
}

// DefaultStrictUpdatePasswordReset clears / replaces / appends first level 1:many children and then executes a gorm update call
func DefaultStrictUpdatePasswordReset(ctx context.Context, in *PasswordReset, db *gorm.DB) (*PasswordReset, error) {
	if in == nil {
		return nil, fmt.Errorf("Nil argument to DefaultStrictUpdatePasswordReset")
	}
	ormObj, err := in.ToORM(ctx)
	if err != nil {
		return nil, err
	}
	lockedRow := &PasswordResetORM{}
	db.Model(&ormObj).Set("gorm:query_option", "FOR UPDATE").Where("token=?", ormObj.Token).First(lockedRow)
	if hook, ok := interface{}(&ormObj).(PasswordResetORMWithBeforeStrictUpdateCleanup); ok {
		if db, err = hook.BeforeStrictUpdateCleanup(ctx, db); err != nil {
			return nil, err
		}
	}
	if hook, ok := interface{}(&ormObj).(PasswordResetORMWithBeforeStrictUpdateSave); ok {
		if db, err = hook.BeforeStrictUpdateSave(ctx, db); err != nil {
			return nil, err
		}
	}
	...
	if hook, ok := interface{}(&ormObj).(PasswordResetORMWithAfterStrictUpdateSave); ok {
		if err = hook.AfterStrictUpdateSave(ctx, db); err != nil {
			return nil, err
		}
	}
...
type PasswordResetORMWithBeforeStrictUpdateCleanup interface {
	BeforeStrictUpdateCleanup(context.Context, *gorm.DB) (*gorm.DB, error)
}
type PasswordResetORMWithBeforeStrictUpdateSave interface {
	BeforeStrictUpdateSave(context.Context, *gorm.DB) (*gorm.DB, error)
}
type PasswordResetORMWithAfterStrictUpdateSave interface {
	AfterStrictUpdateSave(context.Context, *gorm.DB) error
}

// DefaultPatchPasswordReset executes a basic gorm update call with patch behavior
func DefaultPatchPasswordReset(ctx context.Context, in *PasswordReset, updateMask *field_mask.FieldMask, db *gorm.DB) (*PasswordReset, error) {
	if in == nil {
		return nil, errors.NilArgumentError
	}
	var pbObj PasswordReset
	var err error
	if hook, ok := interface{}(&pbObj).(PasswordResetWithBeforePatchRead); ok {
		if db, err = hook.BeforePatchRead(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
	if hook, ok := interface{}(&pbObj).(PasswordResetWithBeforePatchApplyFieldMask); ok {
		if db, err = hook.BeforePatchApplyFieldMask(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
	if _, err := DefaultApplyFieldMaskPasswordReset(ctx, &pbObj, in, updateMask, "", db); err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&pbObj).(PasswordResetWithBeforePatchSave); ok {
		if db, err = hook.BeforePatchSave(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
	pbResponse, err := DefaultStrictUpdatePasswordReset(ctx, &pbObj, db)
	if err != nil {
		return nil, err
	}
	if hook, ok := interface{}(pbResponse).(PasswordResetWithAfterPatchSave); ok {
		if err = hook.AfterPatchSave(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
...
type PasswordResetWithBeforePatchRead interface {
	BeforePatchRead(context.Context, *PasswordReset, *field_mask.FieldMask, *gorm.DB) (*gorm.DB, error)
}
type PasswordResetWithBeforePatchApplyFieldMask interface {
	BeforePatchApplyFieldMask(context.Context, *PasswordReset, *field_mask.FieldMask, *gorm.DB) (*gorm.DB, error)
}
type PasswordResetWithBeforePatchSave interface {
	BeforePatchSave(context.Context, *PasswordReset, *field_mask.FieldMask, *gorm.DB) (*gorm.DB, error)
}
type PasswordResetWithAfterPatchSave interface {
	AfterPatchSave(context.Context, *PasswordReset, *field_mask.FieldMask, *gorm.DB) error
}

// DefaultPatchSetPasswordReset executes a bulk gorm update call with patch behavior
func DefaultPatchSetPasswordReset(ctx context.Context, objects []*PasswordReset, updateMasks []*field_mask.FieldMask, db *gorm.DB) ([]*PasswordReset, error) {
	if len(objects) != len(updateMasks) {
		return nil, fmt.Errorf(errors.BadRepeatedFieldMaskTpl, len(updateMasks), len(objects))
	}

	results := make([]*PasswordReset, 0, len(objects))
	for i, patcher := range objects {
		pbResponse, err := DefaultPatchPasswordReset(ctx, patcher, updateMasks[i], db)
		if err != nil {
			return nil, err
		}
...
// DefaultApplyFieldMaskPasswordReset patches an pbObject with patcher according to a field mask.
func DefaultApplyFieldMaskPasswordReset(ctx context.Context, patchee *PasswordReset, patcher *PasswordReset, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*PasswordReset, error) {
	if patcher == nil {
		return nil, nil
	} else if patchee == nil {
		return nil, errors.NilArgumentError
...
// DefaultListPasswordReset executes a gorm list call
func DefaultListPasswordReset(ctx context.Context, db *gorm.DB) ([]*PasswordReset, error) {
	in := PasswordReset{}
	ormObj, err := in.ToORM(ctx)
	if err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(PasswordResetORMWithBeforeListApplyQuery); ok {
		if db, err = hook.BeforeListApplyQuery(ctx, db); err != nil {
			return nil, err
		}
	}
	db, err = gorm1.ApplyCollectionOperators(ctx, db, &PasswordResetORM{}, &PasswordReset{}, nil, nil, nil, nil)
	if err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(PasswordResetORMWithBeforeListFind); ok {
		if db, err = hook.BeforeListFind(ctx, db); err != nil {
			return nil, err
		}
	}
	...
	if hook, ok := interface{}(&ormObj).(PasswordResetORMWithAfterListFind); ok {
		if err = hook.AfterListFind(ctx, db, &ormResponse); err != nil {
			return nil, err
		}
	}
	pbResponse := []*PasswordReset{}
	for _, responseEntry := range ormResponse {
		temp, err := responseEntry.ToPB(ctx)
		if err != nil {
			return nil, err
...
type PasswordResetORMWithBeforeListApplyQuery interface {
	BeforeListApplyQuery(context.Context, *gorm.DB) (*gorm.DB, error)
}
type PasswordResetORMWithBeforeListFind interface {
	BeforeListFind(context.Context, *gorm.DB) (*gorm.DB, error)
}
type PasswordResetORMWithAfterListFind interface {
	AfterListFind(context.Context, *gorm.DB, *[]PasswordResetORM) error
}
