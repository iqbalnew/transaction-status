...
DOCKER_CONTAINER_NAME?=qcash-template-service
DOCKER_CONTAINER_IMAGE?=qcash-template-service:latest
DOCKER_BUILD_ARGS?=
...
unit-test:
	...
	go test -cover -v -json -count=1 -parallel=4 ./...
	@echo "Unit test done"
...
run:
	go run  ./server/ grpc-gw-server --port1 9225 --port2 3129 --grpc-endpoint :9225
